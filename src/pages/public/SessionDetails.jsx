...
const SessionDetails = () => {
  ...
  }, [isAlreadyBookedSession, isRegistrationOpen, isUpcoming, user?.userEmail]);
  ...
  const onSubmit = async (data) => {
    ...
    const { data: result } = await axiosSecure.post("/add-review", reviewData);
    if (result.success) {
  ...
  return sessionLoading || authLoading || user?.isRoleLoading ? (
    ...
    <div className="max-w-8xl mx-auto mt-10 space-y-12 px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="md:col-span-2">
          ...
          />
          <h1 className="text-3xl font-bold mb-4">{session.sessionTitle}</h1>
